...
class Dense(Layer):
    ...
    def forward(self, x_input):
        self._input = x_input
        if self.w is None:
            self.w = np.random.normal(size=[self.units, x_input.shape[1]])[np.newaxis, :, :] * 0.1
    ...
    def backward(self, w, grad):
        ...
        if isinstance(w, int) and w == -1:
            ...
            self.dz = (w @ grad * dg).reshape(-1,1)
        self.dw = (grad @ np.mean(self._input, axis=0, keepdims=True).reshape(1, -1))[np.newaxis, :, :]
        self.db = 1 / self.m * np.sum(grad, axis=1, keepdims=True)
